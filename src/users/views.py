...
async def get_user_by_id(user_id: uuid.UUID, db: AsyncSession = Depends(get_db),
                         current_user: User = Depends(AuthService.get_current_user_from_token)):
    return await UserService.get_user_by_id(user_id=user_id, db=db, current_user=current_user)
...
async def get_user_by_email(email: str, db: AsyncSession = Depends(get_db),
    ...
    return await UserService.get_user_by_email(email=email, db=db, current_user=current_user)


@user_router.delete("/delete/{user_id}")
async def delete_user_by_id(user_id: uuid.UUID, db: AsyncSession = Depends(get_db),
                            current_user: User = Depends(AuthService.get_current_user_from_token)):
    return await UserService.delete_user_by_id(user_id=user_id, db=db, current_user=current_user)
...
async def update_user_by_id(
    ...
    return await UserService.update_user_by_id(user_id=user_id, user_to_update=user, db=db, current_user=current_user)


@user_router.patch("/soft_delete/{user_id}")
async def soft_delete(user_id: uuid.UUID, db: AsyncSession = Depends(get_db),
                      current_user: User = Depends(AuthService.get_current_user_from_token)):
    return await UserService.soft_delete_user(user_id=user_id, db=db, current_user=current_user)
